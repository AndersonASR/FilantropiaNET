...
@Using (Html.BeginForm()) 
    ...
    <div class="form-group">
        @Html.LabelFor(Function(model) model.TipoEndereco, htmlAttributes:=New With {.class = "control-label col-md-2"})
        <div class="col-md-10">
            @Html.DropDownListFor(Function(model) model.TipoEndereco.ID, ViewBag.Tipos)
            @Html.ValidationMessageFor(Function(model) model.TipoEndereco, "", New With {.class = "text-danger"})
        </div>
    </div>
